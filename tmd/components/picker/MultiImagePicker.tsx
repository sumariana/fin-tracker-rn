...
import { Dimensions, Image, ImageBackground, View, ViewStyle } from "react-native";
...
export default function MultiImagePicker({
  ...
  const handleOpenImagePicker = () => {
    requestPermissions([CAMERA_PERMISSIONS, STORAGE_PERMISSIONS], () => {
      if (selectedImageUrls.some(it=>it.length == 0)) {
        setIsOpen(true);
      }
  ...
  const addImage = (imageUrl: string) => {
    ...
    setSelectedImageUrls(newImageUrls.map(item=>{
      if(item.length == 0 && !replacementDone){
